...
public class WarAllViewActivity extends  AppCompatActivity  { 
	
	private FirebaseDatabase _firebase = FirebaseDatabase.getInstance();
	
	private ArrayList<HashMap<String, Object>> war = new ArrayList<>();
	...
	protected void onCreate(Bundle _savedInstanceState) {
		...
		setContentView(R.layout.war_all_view);
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		_War_child_listener = new ChildEventListener() {
			...
			public void onChildAdded(DataSnapshot _param1, String _param2) {
				...
				War.addListenerForSingleValueEvent(new ValueEventListener() {
					@Override
					public void onDataChange(DataSnapshot _dataSnapshot) {
						...
						key.add(_childKey);
						_grid_view(war);
						listview1.setAdapter(new Listview1Adapter(war));
	...
	public class Listview1Adapter extends BaseAdapter {
		...
		public View getView(final int _position, View _v, ViewGroup _container) {
			...
			View _view = _v;
			if (_view == null) {
				_view = _inflater.inflate(R.layout.war, null);
			}
			
			final LinearLayout linear1 = (LinearLayout) _view.findViewById(R.id.linear1);
			final ImageView imageview1 = (ImageView) _view.findViewById(R.id.imageview1);
			
			Glide.with(getApplicationContext()).load(Uri.parse(war.get((int)_position).get("cover").toString())).into(imageview1);
			linear1.setOnClickListener(new View.OnClickListener() {
				@Override
				public void onClick(View _view) {
					i.setClass(getApplicationContext(), HomeViewActivity.class);
					i.putExtra("k", key.get((int)(_position)));
					i.putExtra("tag", "War");
					startActivity(i);
				}
			});
