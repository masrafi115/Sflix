...
public class CartoomAnimationActivity extends  AppCompatActivity  { 
	...
	private DatabaseReference DB = _firebase.getReference("Cartoon");
	private ChildEventListener _DB_child_listener;
	private Intent i = new Intent();
	@Override
	protected void onCreate(Bundle _savedInstanceState) {
		super.onCreate(_savedInstanceState);
		setContentView(R.layout.cartoom_animation);
	...
	public class Listview1Adapter extends BaseAdapter {
		...
		public View getView(final int _position, View _v, ViewGroup _container) {
			...
			_ToetusAung(icon);
			Glide.with(getApplicationContext()).load(Uri.parse(map.get((int)_position).get("cover").toString())).into(icon);
			linear1.setOnClickListener(new View.OnClickListener() {
				@Override
				public void onClick(View _view) {
					i.setClass(getApplicationContext(), HomeViewActivity.class);
					i.putExtra("k", key.get((int)(_position)));
					i.putExtra("tag", "Cartoon");
					startActivity(i);
				}
			});
